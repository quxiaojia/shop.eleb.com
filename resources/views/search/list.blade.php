...
@section('contents')
    <form class="form-inline"action="{{route('food.index')}}"  method="get" >
            <div class="input-group">
                ...
                <select class="form-control"style="width: 250px;" name="category_id">
                    ...
                    @foreach($foods_class as $food_class)
                        <option value="{{$food_class->id}}"@if($food_class->id == $category_id) selected = "selected"@endif>{{$food_class->name}}</option>
                    @endforeach
                </select>
            </div>
        <div class="form-group">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control" id="exampleInputAmount" value="{{$menu}}" name="menu" placeholder="菜品搜索" style="width: 300px">
            </div>
        </div>

        <div class="input-group">
            <span class="input-group-addon" id="basic-addon1" style="width: 60px;">价格区间: </span>
            <select class="form-control"style="width: 250px;" name="price">
                <option value="1">全部</option>
                    <option value="2" @if($price == 2) selected = "selected"@endif>小于20</option>
                    <option value="3" @if($price == 3) selected = "selected"@endif>20~50</option>
                    <option value="4" @if($price == 4) selected = "selected"@endif>50~100</option>
                    <option value="5" @if($price == 5) selected = "selected"@endif>大于100</option>
            </select>
        </div>
        {{csrf_field()}}
    ...
    <center>{{ $foods->appends(['category_id'=>$category_id,'price'=>$price,'menu'=>$menu])->links() }}</center>
